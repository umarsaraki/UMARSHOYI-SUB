...
const FLW_SECRET_HASH = "UMARSHOYI_SUB_wallet_0891"
...
// 1. FUNCTION DON DUBA PIN - GYARA: wallets maimakon users
...
Deno.serve(async (req) => {  // GYARA: Deno.serve maimakon serve
  ...
  try {
    ...
    // 10. CHANGE PASSWORD - GYARA: wallets
    if(type === "change_password"){
      ...
      await supabase.from('wallets').update({password: hash}).eq('user_id', user_id)
      return new Response(JSON.stringify({success: true, message: "Password changed"}), {headers: corsHeaders})
    }

    // 11. CHANGE TRANSACTION PIN - GYARA: wallets
    if(type === "change_pin"){
    ...
    // 12. BIOMETRIC LOGIN - GYARA: wallets
    ...
    // 13. LOGIN DA BIOMETRIC - GYARA: wallets
